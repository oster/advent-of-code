...
def compact_part2(file_blocks: dict[int, Block], free_blocks: list[Block]) -> tuple[dict[int, Block], list[Block]]:
    for file_id in sorted(file_blocks.keys(), reverse=True):
        ...
        for freeblock_idx in range(last_index_for_size.get(block_size, 0), len(free_blocks)):
            freeblock_start, freeblock_size = free_blocks[freeblock_idx]
            if freeblock_size >= block_size and freeblock_start < block_start:

                last_index_for_size[block_size] = freeblock_idx
                ...
                # else:
                #     free_blocks[freeblock_idx] = (freeblock_start + block_size, 0)
